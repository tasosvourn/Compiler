...
def gnlvcode(x):
    ...
    for i in range(len(scopes)-1, -1, -1):
        ...
        for entity in scopes[i][2]:
            if entity[0] == x:
                levelM = scopes[i][0];
                outfileASM.write("lw $t0, -4($sp)\n")
                for j in range(levelFunc-levelM -1):
                    outfileASM.write("lw $t0, -4($t0)\n")
                outfileASM.write("add $t0, $t0, -"+str(entity[1])+"\n")

                return


def loadvr(v,r):
    global outfileASM, scopes

    v = str(v)
    levelFunc = len(scopes) - 1

    if v[0].isdigit() == True:
        outfileASM.write("li $t"+str(r)+","+v+"\n")
    else:
         for i in range(len(scopes)-1, -1, -1):
            levelM = scopes[i][0];
            for entity in scopes[i][2]:
                if entity[0] == v:
                    if levelM == 0:#global
                        outfileASM.write("lw $t"+str(r)+",-"+str(entity[1])+"($s0)\n")
                    elif levelFunc == levelM:#idio vathos
                        if len(entity)==3 and entity[2] == "REF":#me anafora
                            outfileASM.write("lw $t0,-"+str(entity[1])+"($sp)\n")
                            outfileASM.write("lw $t"+str(r)+",($t0)\n")
                        else:#xwris anafora
                            outfileASM.write("lw $t"+str(r)+",-"+str(entity[1])+"($sp)\n")
                    else:
                        if len(entity)==3 and entity[2] == "REF":#me anafora
                            gnlvcode(v)
                            outfileASM.write("lw $t0,($t0)\n")
                            outfileASM.write("lw $t"+str(r)+",($t0)\n")
                        else:#xwris
                            gnlvcode(v)
                            outfileASM.write("lw $t"+str(r)+",($t0)\n")
...
def storerv(r,v):
    ...
    for i in range(len(scopes)-1, -1, -1):
        ...
        for entity in scopes[i][2]:
            if entity[0] == v:
                if levelM == 0:#global
                    outfileASM.write("sw $t"+str(r)+",-"+str(entity[1])+"($s0)\n")
                elif levelFunc == levelM:#idio vathos
                    if len(entity)==3 and entity[2] == "REF":#me anafora
                        outfileASM.write("lw $t0,-"+str(entity[1])+"($sp)\n")
                        outfileASM.write("sw $t"+str(r)+",($t0)\n")
                    else:#xwris anafora
                        outfileASM.write("sw $t"+str(r)+",-"+str(entity[1])+"($sp)\n")
                else:
                    if len(entity)==3 and entity[2] == "REF":#me anafora
                        gnlvcode(v)
                        outfileASM.write("lw $t0,($t0)\n")
...
def writeIntToAsmFile():
    global asmQuad, quads
    ...
    if len(scopes) == 1:
        outfileASM.write("Lmain:\n")
        frameLength = 12
        for entity in scopes[0][2]:
            if len(entity) == 2 or len(entity) == 3:
                frameLength = frameLength + 4
        outfileASM.write("add $sp, $sp, "+str(frameLength)+"\n")
        outfileASM.write("move $s0, $sp\n")

    for i in range(asmQuad,len(quads)):
        quad = quads[i]

        outfileASM.write("L"+str(quad[0])+":\n")

        if quad[1] == "end block":
            if(len(scopes) != 1):
                outfileASM.write("lw $ra, ($sp)\n")
                outfileASM.write("jr $ra\n")
        elif quad[1] == "begin block":
            if(len(scopes) != 1):
                outfileASM.write("sw $ra, ($sp)\n")
        elif quad[1] == "out":
            outfileASM.write("li $v0, 1\n")
            loadvr(quad[2],0)
            outfileASM.write("move $a0, $t0\n")
            outfileASM.write("syscall\n")
        elif quad[1] == "inp":
            outfileASM.write("li $v0, 5\n")
            outfileASM.write("syscall\n")
            outfileASM.write("move $t0, $v0\n")
            storerv(0,quad[2])
        elif quad[1] == "retv":
            loadvr(quad[2],1)
            outfileASM.write("lw $t0, -8($sp)\n")
            outfileASM.write("sw $t1, ($t0)\n")
        elif quad[1] == "jump":
            outfileASM.write("j L"+str(quad[4])+"\n")
        elif quad[1] == ":=":
           loadvr(quad[2],1)
           storerv(1, quad[4])
        elif quad[1] == "+":
            loadvr(quad[2],1)
            loadvr(quad[3],2)
            outfileASM.write("add $t1, $t1, $t2\n")
            storerv(1, quad[4])
        elif quad[1] == "-":
            loadvr(quad[2],1)
            loadvr(quad[3],2)
            outfileASM.write("sub $t1, $t1, $t2\n")
            storerv(1, quad[4])
        elif quad[1] == "*":
            loadvr(quad[2],1)
            loadvr(quad[3],2)
            outfileASM.write("mul $t1, $t1, $t2\n")
            storerv(1, quad[4])
        elif quad[1] == "/":
            loadvr(quad[2],1)
            loadvr(quad[3],2)
            outfileASM.write("div $t1, $t1, $t2\n")
            storerv(1, quad[4])
        elif quad[1] == "=":
            loadvr(quad[2],1)
            loadvr(quad[3],2)
            outfileASM.write("beq $t1, $t2, L"+str(quad[4])+"\n")
        elif quad[1] == ">":
            loadvr(quad[2],1)
            loadvr(quad[3],2)
            outfileASM.write("bgt $t1, $t2, L"+str(quad[4])+"\n")
        elif quad[1] == "<":
            loadvr(quad[2],1)
            loadvr(quad[3],2)
            outfileASM.write("blt $t1, $t2, L"+str(quad[4])+"\n")
        elif quad[1] == ">=":
            loadvr(quad[2],1)
            loadvr(quad[3],2)
            outfileASM.write("bge $t1, $t2, L"+str(quad[4])+"\n")
        elif quad[1] == "<=":
            loadvr(quad[2],1)
            loadvr(quad[3],2)
            outfileASM.write("ble $t1, $t2, L"+str(quad[4])+"\n")
        elif quad[1] == "<>":
            loadvr(quad[2],1)
            loadvr(quad[3],2)
            outfileASM.write("bne $t1, $t2, L"+str(quad[4])+"\n")
        elif quad[1] == "par":
            if isFirst == True:
                isFirst = False
                j = i+1
                ...
                for k in range(len(scopes)-1, -1, -1):
                    for entity in scopes[k][2]:
                        if entity[0] == callName:
                            found = True
                            outfileASM.write("add $fp, $sp, "+str(entity[3])+"\n")
                            break
                    if found == True:
                        break
            if quad[3] == "CV":
                loadvr(quad[2],0)
                x = -(12+4*k)
                outfileASM.write("sw $t0, "+str(x)+"($fp)\n")
            elif quad[3] == "RET":
                for entity in scopes[len(scopes)-1][2]:
                    if entity[0] == quad[2]:
                        outfileASM.write("add $t0, $sp, -"+str(entity[1])+"\n")
                        break
                outfileASM.write("sw $t0, -8($fp)\n")
            k = k+1
        elif quad[1] == "call":
            isFirst = True
            outfileASM.write("sw $sp, -4($fp)\n")
            callName = quads[j][2]
            ...
            for k in range(len(scopes)-1, -1, -1):
                for entity in scopes[k][2]:
                    if entity[0] == callName:
                        found = True
                        outfileASM.write("add $sp, $sp, "+str(entity[3])+"\n")
                        outfileASM.write("jal L"+str(entity[2])+"\n")
                        outfileASM.write("add $sp, $sp, -"+str(entity[3])+"\n")
                        break
                if found == True:
                    break
...
if len(name) == 1:
    ...
    openName = ".".join(name[0:len(name)-1]) + ".asm"
outfileASM = open(openName, "w")
outfileASM.write("j Lmain\n")
